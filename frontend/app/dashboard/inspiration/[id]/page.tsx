...
export default function SpirateDetail() {
  ...
  const handleChoiceSelect = async (choice: string) => {
    ...
    try {
      ...
      if (inspiration && inspiration.id) {
        // 将用户选择记录到数据库
        const currentContent = updatedDisplayedLines.join('\n\n');
  ...
  const typewriterEffectForContinuation = async (lines: string[], startIndex: number) => {
    ...
    // 保存当前显示的内容，包括用户选择
    const currentLines = [...displayedLines];
    
    // 用于存储最终要保存的完整内容
    let finalStoryContent = [...currentLines];
    ...
    //   try {
    //     // 包含用户选择记录和新内容
    //     const completeContent = [...finalStoryContent].join('\n\n');
    ...
    //       story_direction: inspiration.story_direction
    //     });
    //     console.log('新生成的故事内容已保存到数据库');
    //   } catch (error) {
    //     console.error('保存新生成的故事内容失败:', error);
  ...
  // 为缺少图片的角色生成图片----------------------------------------------------------------------------------------------------
  const generateMissingCharacterImages = (characters: Character[]) => {
    if (!characters || characters.length === 0) return;

    console.log("检查角色图片状态", characters);

    // 筛选出没有图片的角色
    const charactersWithoutImages = characters.filter(char =>
      !char.image_url || char.image_url.trim() === ''
    );

    if (charactersWithoutImages.length === 0) {
      console.log("所有角色已有图片，无需生成");
      return;
    }

    console.log("发现没有图片的角色:", charactersWithoutImages.length);

    // 为每个没有图片的角色生成图片
    const imagePromises = charactersWithoutImages.map(async (character) => {
      try {
        const user = JSON.parse(localStorage.getItem('user') || '{}');
        console.log("开始为角色生成图片:", character.name);
        ...
        // 更新角色信息
        const updatedCharacter = await apiService.character.update(character.id, json_data);
        console.log("角色图片更新成功:", character.name, image_url);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // 检查角色是否都有图片，如果没有则生成------------------------------------------------------------------
        generateMissingCharacterImages(responseData.characters);
        //----------------------------------------------------------------------------------------------------
  ...
  // 移除不必要的 useEffect，因为我们已经在其他地方处理了内容保存
          ...
          {characters.map((char, idx) => (
            <div key={idx} className="flex flex-col items-center">
              ...
              <Button size="sm" variant="outline" onClick={async () => {
                ...
                await apiService.character.update(char.id, json_data);
          ...
          {displayedLines.map((line, index) => (
            <div key={index} className="text-gray-300">
              {editingLineIndex === index ? (
                ...
                // 渲染用户选择提示
                <div
                  className="border-l-4 border-yellow-500 pl-3 py-2 my-2 text-yellow-400 font-medium"
                >
