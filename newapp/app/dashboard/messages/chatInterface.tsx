...
import { SendHorizontal, Info, Smile, Loader2 } from 'lucide-react';
...
const ChatInterface = () => {
  ...
  const [initialCharacterId, setInitialCharacterId] = useState<string | null>(null);
...
//   const [sessions, setSessions] = useState<Session[]>([]);
  ...
  const switchCharacter = async (character: Character,sessionID:string) => {
    setCurrentCharacter(character);
    const existingSession = sessionsResponse.find(
          ...
          <div className="space-y-4">
            {characters.map(character => (
              <div 
                ...
                onClick={() => switchCharacter(character,character.session_id)}
              >
